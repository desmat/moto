...
- **Mileage-ordering guard** (S0 review fix — mileage-type logs are overwrite-always, so POST order matters): the proposal schema/prompt constrains the model to **at most one `mileage`-type log — the current reading**; historical odometer values ride as `mileage` on backdated `service` logs (where S11's monotonic rule makes ordering irrelevant). Belt-and-braces, the confirm handler doesn't trust the model: it POSTs the rows sorted so any `mileage`-type log goes **last** (and if the model ever emits several despite the prompt, keeps only the newest-dated one, dropping the rest from the review table before confirm).
...
- `services/ai.ts`: `chatJSON(...)` + `onboarding` scripted mock. (Handover: canned responses live in `test/fixtures/ai-mocks.json`, not a `MOCKS` const. A script isn't a single static value, so store the `onboarding` entry as an **array of turn objects** and have `chatJSON`'s mock branch index it by the number of user messages in the transcript — the registry stays data-only, the tiny turn-picking logic lives in code.)
