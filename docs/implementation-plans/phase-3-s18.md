...
- **`AI_MOCK` is a scripted turn**: emits one `tool` event (`get_maintenance_status`) then streams a canned answer that quotes the seeded overdue chain item — deterministic e2e coverage of the whole SSE render path.
